...
namespace Real_Estate.Infrastructure.Identity.Services
{
	public class AccountService : IAccountService
	{
            ...
            Roles typeOfUser)
		{
			...
			if (typeOfUser.ToString() == Roles.Client.ToString())
			{
				...
				await _emailService.SendEmailAsync(new EmailRequest()
				{
					...
					Subject = "Confirm account of Real Estate"
				});
			}
		...
		public async Task<string> ConfirmAccountAsync(string userId, string token)
		{
			...
			if (result.Succeeded)
			{
				return $"Account confirmed for {user.Email}. You can now use the Real Estate";
			}
			else
			{
			...
			string origin)
		{
			...
			{
				To = user.Email,
				Body = $"Please reset your password by clicking this URL {verificationUri} from the Real Estate",
		...
		private async Task<string> SendForgotPasswordUri(ApplicationUser user, string origin)
		{
			...
			var verificationUri = QueryHelpers.AddQueryString(Uri.ToString(), 
				"token", code);
