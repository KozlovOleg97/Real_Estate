...
namespace Real_Estate.Core.Application.Features.Agents.Queries.GetAllAgentProperties
{
    ...
    public class GetAllCategoriesQueryHandler : IRequestHandler<GetAllAgentPropertiesQuery, 
        IEnumerable<PropertiesViewModel>>
    {
        ...
        public async Task<IEnumerable<PropertiesViewModel>> Handle(GetAllAgentPropertiesQuery query, 
            CancellationToken cancellationToken)
        {
            var properties = await _propertiesRepository.GetAllWithIncludeAsync(
                new List<string> { "Improvements", "TypeOfProperty", "TypeOfSale" });

            var propertiesByAgentId = properties.Where(x => x.AgentId == query.Id);

            if (propertiesByAgentId == null || propertiesByAgentId.Count() == 0) 
